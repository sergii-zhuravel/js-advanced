...
//!!! Конструктор использует неправильньіе названия для параметров функции
// Нужно например function Pizza(toppings, bortik , size)
//  и потом уже их назначать в поля обьекта
function Pizza(array, boolean, string) {
  ...
  // что такое argument? агрумент не используется
...
function applyAll() {
  ...
  //!!! можно в одну строку
  // return arguments[0].apply(null, arguments.slice(1,array.length));
}
...
console.log(
  add_ingridients.apply(pizza1, ["toppings", "pineapple", "cucumber"])
);
